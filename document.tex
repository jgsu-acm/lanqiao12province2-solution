...
\begin{frame}
    ...
    当然，也可以推出此递推式的通项公式 $S_{len}=2^{len-1}(3len-4) - len + 2$ 来 $O(1)$ 出结果
\end{frame}
...
\begin{frame}
    ...
    注意到出现了有理数取余，需要预处理阶乘及其逆元 $\left[ \cfrac{n!}{len!} \equiv n! \times (len!)^{-1}\pmod p \right]$。数据规模 $n \leq {10}^6$ 不是很大，采用 $O(n)$ 或者 $O(n\log{n})$ 求逆元都可以。若采用 $O(n\log{n})$ 求逆元则总时间复杂度为 $O(n\log{n}+T)$
\end{frame}
