...
\begin{frame}{题解}
    ...
    发现直接递推的话，对于每个数都需要 $O(\sqrt{n})$ 找约数，时间复杂度是 $O(n\sqrt{n})$ 的，有些慢。于是我们采用刷表法优化，用前向状态去更新后继状态，即用每个数的 DP 值去更新其倍数的 DP 值。过程类似埃氏筛
...
\begin{frame}{题解}
    ...
    \begin{itemize}
        ...
        \item 转移方程：$$dp[i][p][q][j] = \sum_{a} dp[i-1][q][a][j-popcount(p)]~,j \in [popcount(p), k],~(q,p),(a,p) \text{不冲突}$$
